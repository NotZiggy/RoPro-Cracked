...
profileThemeBoxHTML = `<div class="section-content"><button id="saveTheme" type="button" class="btn-fixed-width-lg btn-growth-lg" style="background-color:#0084dd;border:0px;font-size:18px;padding:5px;float:right;">${chrome.i18n.getMessage("RemoveTheme")}</button><p id="themeResponseText"></p><div style="width:100%;float:left;margin-top:10px;"><h3 style="margin-bottom:-10px;">${chrome.i18n.getMessage("RoProProfileThemes")}</h3><h7 style="font-size:14px;margin-top:-10px;">${chrome.i18n.getMessage("OtherRoproUsersWillSeeYourThemeWhenTheyVisitYourProfile")}</h7><br><br></div><div style="min-height:150px" id="profileThemeCardBox"></div><br><div style="width:100%;float:left;margin-top:10px;"><h3 style="margin-bottom:-10px;">${chrome.i18n.getMessage("AnimatedThemes")}</h3><h7 style="font-size:14px;margin-top:-10px;">${chrome.i18n.getMessage("AnimatedThemesAreAvailableForSubscribersOnly")}</h7><br><br><div style="min-height:110px;" id="animatedThemeCardBox"></div></div></div>`
mainProfileAnimationHTML = `<iframe style="position:absolute;width:100%;height:300%;top:-100%;left:0px;border:none;pointer-events:none;" src="https://ropro.io/animatetest01/?user=${getIdFromURL(location.href)}&mainicon"></iframe>`
circularProfileAnimationHTML = `<div style="position:absolute;top:-0px;left:0px;width:100%;height:100%;overflow:hidden!important;border-radius:100px;pointer-events:none;"><iframe style="width:100%;height:100%;border:none;" src="https://ropro.io/animatetest01/?user=${getIdFromURL(location.href)}&circleicon"></iframe></div>`
...
function addOpenAnimationsButton(){
	div = document.getElementsByClassName('profile-themes-button')[0]
	div2 = document.createElement('div')
	div2.innerHTML += `<a id="openAnimations" style="margin-left:10px;display:inline-block;" class="btn-primary-md ng-binding">Animations<img style="margin:-5px;margin-top:-7px;margin-right:-2px;margin-left:3px;width:20px;${$('.light-theme').length == 0 ? "" : "filter: invert(1);"}" src="https://ropro.io/images/animation_icon2.png"></a>`
	div.appendChild(div2.childNodes[0])
	document.getElementsByClassName('header-title')[0].appendChild(div)
	document.getElementById('openAnimations').addEventListener('click', function(){
		if (this.innerHTML == `Animations<img style="margin:-5px;margin-top:-7px;margin-right:-2px;margin-left:3px;width:20px;${$('.light-theme').length == 0 ? "" : "filter: invert(1);"}" src="https://ropro.io/images/animation_icon2.png">`) {
			//loadAnimations()
			this.innerHTML = `Close Animations<img style="margin:-5px;margin-top:-7px;margin-right:-2px;margin-left:3px;width:20px;${$('.light-theme').length == 0 ? "" : "filter: invert(1);"}" src="https://ropro.io/images/animation_icon2.png">`
		} else {
			this.innerHTML = `Animations<img style="margin:-5px;margin-top:-7px;margin-right:-2px;margin-left:3px;width:20px;${$('.light-theme').length == 0 ? "" : "filter: invert(1);"}" src="https://ropro.io/images/animation_icon2.png">`
			document.getElementById('profileAnimationsSelectorBox').remove()
		}
	})
}

function addAnimationFrames() {
	div = document.createElement('div')
	div.innerHTML = circularProfileAnimationHTML
	document.getElementsByClassName('profile-avatar-image')[0].appendChild(div.childNodes[0])
	document.getElementsByClassName('profile-avatar-image')[0].getElementsByClassName('thumbnail-2d-container')[0].style.display = "none"
	div = document.createElement('div')
	div.innerHTML = mainProfileAnimationHTML
	document.getElementsByClassName('thumbnail-holder')[0].appendChild(div.childNodes[0])
	document.getElementsByClassName('thumbnail-holder')[0].getElementsByClassName('thumbnail-2d-container')[0].style.display = "none"
	document.getElementsByClassName('avatar-status')[0].style.zIndex = "10000"
}
